...
import analysis.literal.LiteralAnalysis;
import analysis.type.TypeAnalysis;
import conversion.includes.IncludeGraph;
import conversion.nodes.CfgNodeInclude;
...
public class ProgramConverter {
	...
	public ProgramConverter(boolean specialNodes,
		...
		// this.CreateIncludeFileIncludes(MyOptions.entryFile);
			...
			if (child.isDirectory()) {
				...
				if (i > 0) {
					...
					if (extension.toLowerCase().equalsIgnoreCase("php")) {
						// System.out.println("require_once
						// ('"+child.getPath()+"');");
						if (!allFiles.contains(child)) {
							// System.out.println("including "+child.getName());
	...
	public void convert() {
		...
		try {
			...
			// Set<CfgNodeInclude> notFoundLiteralIncludes = new
			// HashSet<CfgNodeInclude>();
			...
			// Set<CfgNodeInclude> notFoundDynamicIncludes = new
			// HashSet<CfgNodeInclude>();
			...
			while (goOn && !MyOptions.optionW) {
				...
				goOn = false;
				boolean nonLiteralIncludes = false; // are there non-literal
													// includes?
				...
				for (File currentfile : this.allFiles) {
					...
					case INCLUDED:
						// fine!
						/*
						 * BufferedWriter out = null; try { FileWriter fstream =
						 * new FileWriter("out.txt", true); //true tells to
						 * append data. out = new BufferedWriter(fstream);
						 * out.write("include('"+ currentfile+"');");
						 * out.newLine(); } catch (IOException e) {
						 ...
						 * e.printStackTrace(); } } }
						 */
						// System.out.println("include('"+ currentfile+"');\n");
						resolvedLit++;
						break;
					case NOTFOUND:
						// notFoundLiteralIncludes.put(includeNode,
						// includeNode.getIncludeMe().toString());
						break;
					case CYCLIC:
				...
				while (!processUs.isEmpty()) {
					...
					for (Iterator<CfgNodeInclude> iter = processUs.iterator(); iter.hasNext();) {
				...
				// assign a reasonable (worklist) order to the nodes, speeds up
				// analyses;
				// not necessary here! this is only needed for functional
				// analyses!
				// baseTac.assignReversePostOrder();

				literalAnalysis = new LiteralAnalysis(baseTac, aliasAnalysis, new CSAnalysis(connectorComp), workList);
				...
				for (CfgNodeInclude includeNode : processUs) {
					...
					// System.out.println("processing include node: " +
					// includeNode.getOrigLineno());
					...
					switch (status) {
					case INCLUDED:
						// we have included something, so continue with the next
						// iteration
						goOn = true;
						resolvedNonLit++;
						break;
				...
				// resolve again (maybe we've got some new calls and functions
				// during this pass);
				// this is unnecessary here...
				// this.baseTac.backpatch();

				processUs = weComeAfterwards;
				processUs.addAll(topIncludes); // maybe they will become
												// resolvable
			...
			if (this.useAliasAnalysis) {
				...
				// EFF: perform type analysis conditionally; currently, type
				// analysis
				...
				connectorComp.compute();
				InterWorkList workList = new InterWorkListBetter(new InterWorkListOrder(baseTac, connectorComp));
				this.typeAnalysis = new TypeAnalysis(this.baseTac, new CSAnalysis(connectorComp), workList);
	...
	private void removeUnreachables(Set<CfgNodeInclude> includeSet, Map<CfgNodeInclude, String> includeMap) {
		...
		for (Iterator<Map.Entry<CfgNodeInclude, String>> iter = includeMap.entrySet().iterator(); iter.hasNext();) {
			Map.Entry<CfgNodeInclude, String> entry = (Map.Entry<CfgNodeInclude, String>) iter.next();
	...
	private File makeFile(String fileName, File includingFile) {
		...
		if (!(fileName.startsWith("./") || fileName.startsWith("../"))) {
			for (Iterator<File> iter = MyOptions.includePaths.iterator(); iter.hasNext();) {
				...
				// we've already dealt with absolute include paths in the
				// previous loop
				if (includePath.isAbsolute()) {
					continue;
				}
				...
				try {
					...
					findMe = new File(searchIn, fileName);
					// System.out.println("probing file " +
					// findMe.getAbsolutePath());
					if (findMe.isFile()) {
						// found it!
						// System.out.println("found it!");
						return findMe;
		...
		// included by " + includingFile.getAbsolutePath());
	}
	...
	public IncStatus include(File includedFile) {
		...
		// if this is a recursive include (include graph would get cyclic),
		// ignore it
		// boolean acyclic =
		// this.includeGraph.addAcyclicEdge(includeNode.getFile(),
		// includedFile);
		...
		try {
			...
			// System.err.println(includedFile);
		} catch (IOException e) {
			throw new RuntimeException(e.getMessage());
		}
		...
		// System.out.println("from: " + includeNode.getFileName() + ", line " +
		// includeNode.getOrigLineno());
		if (!MyOptions.optionB) {
			System.out.print(".");
		}
		try {
			// System.err.println("Current parsed file" + includedFilePath);
			ParseTree parseTree = this.parse(includedFilePath);
			// System.out.println("parsed"+ includedFilePath);
			TacConverter tac = new TacConverter(parseTree, this.specialNodes, this.numConvertedFiles++, includedFile,
					this);
			// System.err.println("converted files" + numConvertedFiles);
			tac.convert();
	...
	public IncStatus include(String includedFileName, CfgNodeInclude includeNode, TacFunction function,
		...
		try {
			includedFilePath = includedFile.getCanonicalPath();
			// System.err.println(includedFileName);
		} catch (IOException e) {
			throw new RuntimeException(e.getMessage());
		}
