...
import conversion.nodes.CfgNodeEval;
import sanit.SanitAnalysis;
import transduction.MyTransductions;

// command execution analysis
public class CodeEvaluatingAnalysis extends DepClient {
	...
	public CodeEvaluatingAnalysis(DepAnalysis depAnalysis) {
		super(depAnalysis);
	...
	public List<Integer> detectVulns() {
		...
		// System.out.println();
		// System.out.println("*****************");
		// System.out.println("Code Evaluation BEGIN");
		// System.out.println("*****************");
		// System.out.println();
		...
		try {
			...
			// System.out.println("Code Evaluation Output");
			// System.out.println("--------------------");
			// System.out.println();
			...
			for (Sink sink : sinks) {
				...
				for (DepGraph depGraph : depGraphs) {
					...
					DepGraph xpathGraph = new DepGraph(depGraph);
					...
					if (auto.hasIndirectlyTaintedTransitions()) {
						if (auto.hasDangerousIndirectTaint()) {
							// System.out.println("indirectly tainted and
							// dangerous!");

							tainted = true;
						} else {
							// System.out.println("indirectly tainted, but not
							// dangerous");
						}
					}
					if (!tainted) {
					...
					if (tainted) {
						...
						if (!dangerousUninit.isEmpty()) {
							if (dangerousUninit.values().contains(InitialTaint.ALWAYS)) {
								...
								// System.out.println("- conditional on
								// register_globals=on");
			...
			// initial sink count and final graph count may differ (e.g., if
			// some sinks
			// are not reachable)
			...
			// System.out.println();
			// System.out.println("*****************");
			// System.out.println("Code Evaluation Analysis END");
			// System.out.println("*****************");
			// System.out.println();
			bw.append("<tfoot>" + "<tr>"
	...
	Automaton toAutomaton(DepGraph depGraph, DepGraph origDepGraph) {
		...
		// BEWARE: minimization can lead to an automaton that is *less*
		// human-readable
		// rootDeco.minimize();
		return rootDeco;
	}
	...
	private void decorate(DepGraphNode node, Map<DepGraphNode, Automaton> deco, Set<DepGraphNode> visited,
			DepGraph depGraph, DepGraph origDepGraph) {
		...
		if (node instanceof DepGraphNormalNode) {
			...
			// for SCC nodes, we generate a coarse string approximation (.*
			// automaton);
			// the taint value depends on the taint value of the successors:
			// if any of the successors is tainted in any way, we make the
			// resulting
			// automaton tainted as well

			/*
			 * this approach works under the assumption that the SCC contains no
			 * "evil" functions (functions that always return a tainted value),
			 * and that the direct successors of the SCC are not <uninit> nodes
			 * (see there, below); it is primarily based on the fact that SCCs
			 * can never contain <uninit> nodes, because <uninit> nodes are
			 * always leaf nodes in the dep graph (since they have no
			 * successors); under the above assumptions and observations, it is
			 * valid to say that the taint value of an SCC node solely depends
			 * on the taint values of its successors, and that is exactly what
			 * we do here
			 *
			 */
			...
			if (pre instanceof DepGraphNormalNode) {
				...
				// this case can really happen (e.g.: dcpportal: advertiser.php,
				// forums.php);

				// take a look at the "real" predecessors (i.e., take a look
				// "into"
				// the SCC node): if there is exactly one predecessor, namely a
				// DepGraphNormalNode, and if the contained place is initially
				// untainted,
				// there is no danger from here; else: we will have to set it to
				// tainted
	...
	private Automaton makeAutoForOp(DepGraphOpNode node, Map<DepGraphNode, Automaton> deco, DepGraph depGraph) {
	...
	protected void checkForSink(CfgNode cfgNodeX, TacFunction traversedFunction, List<Sink> sinks) {
		...
		if (cfgNodeX instanceof CfgNodeCallBuiltin) {
			...
			List<TacActualParam> params = new ArrayList<TacActualParam>();
			TacActualParam param = new TacActualParam(cfgNode.getRight(), false);
			params.add(param);
	...
	private void checkForSinkHelper(String functionName, CfgNode cfgNode, List<TacActualParam> paramList,
	...
	private Transition.Taint multiDependencyAuto(List<DepGraphNode> succs, Map<DepGraphNode, Automaton> deco,
