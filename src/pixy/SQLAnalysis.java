...
extends DepClient {
	...
	public List<Integer> detectVulns() {
		...
		try{
			...
			//bw.write("Number of sinks: " + sinks.size());
			...
			for (Sink sink : sinks) {
				...
				for (DepGraph depGraph : depGraphs) {
					...
					if (auto.hasDirectlyTaintedTransitions()) {
						...
						//bw.write("directly tainted!");
						//bw.write("<td>directly tainted</td>");
						tainted = true;
					} 
					if (auto.hasIndirectlyTaintedTransitions()) {
						if (auto.hasDangerousIndirectTaint()) {
							...
							//bw.write("indirectly tainted and dangerous!");
							//bw.write("<td>indirectly tainted - dangerous!</td>");
							tainted = true;
						} else {
							//bw.write("<td>indirectly tainted - not dangerous!</td>");
					...
					if (!tainted) {
						...
						//bw.write("- Graphs: sql" + graphcount);
					}					
