...
extends DepClient {
	...
	public CommandExecutionAnalysis(DepAnalysis depAnalysis) {
		super(depAnalysis);
	...
	public List<Integer> detectVulns() {
		...
		//System.out.println();
		//System.out.println("*****************");
		//System.out.println("Command Execution BEGIN");
		//System.out.println("*****************");
		//System.out.println();
		...
		try{
			...
			System.out.println("Creating Command Execution Analysis Report");
			bw.append("<p class=\"title\">Command Execution Analysis Report</p>");
			...
			bw.append("<table id=\"rounded-corner\" summary=\"XSS Scan Report\" >"+
					"<thead>"+
					"<tr>"+
			...
			//System.out.println("Command Execution Output");
			//System.out.println("--------------------");
			//System.out.println();
			...
			for (Sink sink : sinks) {
				...
				for (DepGraph depGraph : depGraphs) {
					...
					String graphNameBase = "cmdexec_" + fileName + "_" + graphcount;

					DepGraph xpathGraph = new DepGraph(depGraph);
					if(depGraph.getRoot()==null||depGraph.getRoot().getCfgNode()==null){
						continue;
					}
					...
					if (!tainted) {
						...
						bw.append("<tr>");
						bw.append("<td>Command Execution</td>");
					...
					if (tainted) {
						...
						if (!dangerousUninit.isEmpty()) {
							...
							//bw.newLine();
							bw.append("<td>cmdexec"+graphcount+"</td>");
			...
			//System.out.println();
			//System.out.println("*****************");
			//System.out.println("Command Execution Analysis END");
			//System.out.println("*****************");
			//System.out.println();
			bw.append("<tfoot>"+
			"<tr>"+
	...
	private Automaton makeAutoForOp(DepGraphOpNode node, Map<DepGraphNode,Automaton> deco,
		...
		if (!node.isBuiltin()) {
			...
			// in cases where the output is not enclosed by quotes in an Xpath query
